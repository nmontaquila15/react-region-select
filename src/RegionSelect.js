...
class RegionSelect extends Component {
  ...
  onDocMouseTouchMove(event) {
    ...
    if (!regionChangeData.isMove) {
      ...
      x1Pc =
        ((regionChangeData.clientPosXStart - regionChangeData.imageOffsetLeft) / regionChangeData.imageWidth) *
        100;
      y1Pc =
        ((regionChangeData.clientPosYStart - regionChangeData.imageOffsetTop) / regionChangeData.imageHeight) *
        100;
  ...
  onComponentMouseTouchDown(event) {
    ...
    const clientPos = this.getClientPos(event);
    const imageOffset = this.getElementOffset(this.imageRef.current);
    const xPc = ((clientPos.x - imageOffset.left) / this.imageRef.current.offsetWidth) * 100;
    const yPc = ((clientPos.y - imageOffset.top) / this.imageRef.current.offsetHeight) * 100;
    ...
    if (this.props.regions.length < this.props.maxRegions) {
      ...
      this.props.onChange([...this.props.regions.slice(0, this.props.maxRegions - 1), rect]);
  ...
  onRegionMoveStart(event, index) {
    ...
    const regionLeft = (currentRegion.x / 100) * this.imageRef.current.offsetWidth + imageOffset.left;
    const regionTop = (currentRegion.y / 100) * this.imageRef.current.offsetHeight + imageOffset.top;
    const regionWidth = (currentRegion.width / 100) * this.imageRef.current.offsetWidth;
    const regionHeight = (currentRegion.height / 100) * this.imageRef.current.offsetHeight;
