...
var RegionSelect = (function (_Component) {
  ...
  function RegionSelect(props) {
    ...
    var _this = _possibleConstructorReturn(
      this,
      (RegionSelect.__proto__ || Object.getPrototypeOf(RegionSelect)).call(this, props)
    );
  ...
  _createClass(RegionSelect, [
    ...
    },
    {
      ...
      value: function onDocMouseTouchMove(event) {
        ...
        if (!regionChangeData.isMove) {
          ...
          x =
            ((clientPos.x + regionChangeData.clientPosXOffset - regionChangeData.imageOffsetLeft) /
              regionChangeData.imageWidth) *
            100;
          y =
            ((clientPos.y + regionChangeData.clientPosYOffset - regionChangeData.imageOffsetTop) /
              regionChangeData.imageHeight) *
            100;
        ...
        this.props.onChange(
          [].concat(
            _toConsumableArray(this.props.regions.slice(0, index)),
            [(0, _objectAssign2.default)({}, updatingRegion, rect)],
            _toConsumableArray(this.props.regions.slice(index + 1))
          )
        );
      },
    },
      ...
      value: function onDocMouseTouchEnd() {
        if (this.isChanging) {
          ...
          this.props.onChange(
            [].concat(
              _toConsumableArray(this.props.regions.slice(0, index)),
              [(0, _objectAssign2.default)({}, updatingRegion, changes)],
              _toConsumableArray(this.props.regions.slice(index + 1))
            )
          );
        }
    ...
    },
    {
      ...
      value: function onComponentMouseTouchDown(event) {
        if (
          ...
          isSubElement(event.target, function (el) {
        ...
        var clientPos = this.getClientPos(event);
        var imageOffset = this.getElementOffset(this.imageRef.current);
        var xPc = ((clientPos.x - imageOffset.left) / this.imageRef.current.offsetWidth) * 100;
        var yPc = ((clientPos.y - imageOffset.top) / this.imageRef.current.offsetHeight) * 100;
        ...
        if (this.props.regions.length < this.props.maxRegions) {
          ...
          this.props.onChange(
            [].concat(_toConsumableArray(this.props.regions.slice(0, this.props.maxRegions - 1)), [rect])
          );
    ...
    },
    {
      ...
      value: function onRegionMoveStart(event, index) {
        ...
        var regionLeft = (currentRegion.x / 100) * this.imageRef.current.offsetWidth + imageOffset.left;
        var regionTop = (currentRegion.y / 100) * this.imageRef.current.offsetHeight + imageOffset.top;
        var regionWidth = (currentRegion.width / 100) * this.imageRef.current.offsetWidth;
        var regionHeight = (currentRegion.height / 100) * this.imageRef.current.offsetHeight;
                ...
                _react2.default.createElement(
                  ...
                  regions.map(function (rect, index) {
                    return _react2.default.createElement(
                      'tr',
                      { key: index },
                      _react2.default.createElement('td', null, 'x: ', Math.round(rect.x, 1)),
                      _react2.default.createElement('td', null, 'y: ', Math.round(rect.y, 1)),
                      _react2.default.createElement('td', null, 'width: ', Math.round(rect.width, 1)),
                      _react2.default.createElement('td', null, 'height: ', Math.round(rect.height, 1))
                    );
                  })
                )
              )
